...
const ProductCard = ({ product }) => {
  ...
  let sizeTagClasses =
    "absolute bottom-4 left-4 py-1 px-3 rounded-full text-xs font-bold uppercase tracking-wider";
  let sizeText = "";
  ...
  return (
    console.log("Rendering ProductCard for:", product.slug),
    <Link to={`/producto/${product.slug}`} className="flex flex-col group">
      <div className="relative group h-100 sm:h-120 w-full flex flex-col rounded-lg shadow-md hover:shadow-xl transition-all duration-300 ease-in-out bg-transparent">
        ...
        {sizeText && <div className={sizeTagClasses}>{sizeText}</div>}
      </div>
      {/* Nombre y precio */}
